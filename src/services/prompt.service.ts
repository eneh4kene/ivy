...
class PromptService {
  ...
  private behaviouralAdapter(ctx: Record<string, any>): string {
    ...
    if (blockers.length) {
      ...
      lines.push(
        `ASK ONCE, THEN LET IT GO: if you press for a specific commitment and they deflect ("I'll let you know", "I'll see how I feel"), you may push ONCE. If they deflect again, ACCEPT IT and close warmly — "fair enough, tell me tomorrow" — then end the call. Never ask a third time. Observed live: a member deflected, was pushed twice more, stopped answering, and the call died in silence; his last experience of you was being harangued. A vague answer you accepted costs one day. Making someone dread the phone costs all of them.`
      );
      lines.push(
        `RECURRING BLOCKERS with real counts: ${bl}. When one surfaces again, do NOT merely name it. Name it once plainly ("that's the 3rd time work has eaten the evening"), then PROPOSE ONE CONCRETE ADJUSTMENT and get a yes or no: "So Thursdays don't work after six — want to try Thursday mornings, or make Thursday a rest day and move the session to Friday?" Offer a choice of two, never an open question: someone tired at 9pm cannot design a solution, but they can pick one. If they decline both, accept it at once and ask what they think would work; never propose the same adjustment twice if it was refused. If they agree, say the new arrangement back to them in one sentence so it is unambiguous.${coachGuard} TODAY IS ${ctx.day_of_week ?? 'unknown'} — check that before attributing a miss to a particular day. Saying "that's the second time Monday has eaten it" on a Sunday is exactly the kind of confident error that destroys trust in every count you cite. If the blocker is not tied to today, describe the pattern without pinning it to today ("Mondays keep eating this one").`
      );
    }
