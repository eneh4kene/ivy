...
function VoiceNoteCard({ vn }: { vn: VoiceNote }) {
  ...
  return (
    <div className="surface border-gold-400/25 rounded-2xl p-4">
      <div className="flex items-start gap-2.5">
        <div className="w-8 h-8 rounded-xl bg-[#46f0c8]/12 border border-[#46f0c8]/25 flex items-center justify-center shrink-0">
          <svg viewBox="0 0 16 16" fill="none" className="w-4 h-4 text-gold-300" stroke="currentColor" strokeWidth={1.5}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-1">
            <span className="text-2xs font-bold uppercase tracking-widest text-gold-300">
              This morning you said
            </span>
            {vn.duration > 0 && <span className="text-2xs text-ink-400 font-mono">{dur}</span>}
          </div>
          {vn.transcript ? (
            <p className="text-sm text-ink-50 leading-relaxed italic">
  ...
  return (
    <div className="h-full overflow-y-auto overscroll-contain px-4 py-4 space-y-4">
      {voiceNote ? (
        ...
        <div className={`surface rounded-2xl p-4 ${isArmed ? 'border-sage-400/30' : 'border-ember-400/30'}`}>
      ...
      <div className="surface rounded-2xl p-4 space-y-3">
        <p className="text-xs font-bold uppercase tracking-widest text-ink-400">Your evening reflection</p>
        ...
        {callState === 'requested' ? (
          <div className="flex items-center gap-2 px-4 py-3 rounded-2xl bg-[#2dd4bf]/10 border border-[#2dd4bf]/30">
            <Check className="w-4 h-4 text-sage-300" strokeWidth={2.5} />
          ...
          <button
            ...
            className="relative overflow-hidden w-full flex items-center justify-center gap-2 py-3 rounded-2xl bg-gold-400 text-ink-900 font-semibold
